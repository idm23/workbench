...
#: Web Push identifies the sender with a keypair rather than a shared secret,
#: so these are this machine's identity to every push service it talks to. The
#: private key signs; the public key is handed to the browser and travels back
#: inside each subscription. Generated by the installer — unlike the two
#: credentials that need a browser login, nothing here needs a person.
#:
#: Both are base64url of the raw key material, which is the form `py_vapid`
#: round-trips and the form a browser wants for `applicationServerKey`. PEM
#: would be the obvious choice and is the wrong one: it is multi-line, and
#: these live in an `EnvironmentFile`.
def vapid_private_key() -> str | None:
    return os.environ.get("WORKBENCH_VAPID_PRIVATE_KEY", "").strip() or None
...
def vapid_subject() -> str:
    ...
    default identifies the machine without inventing a mailbox — the same
    reasoning as `agent_git_identity`, with one difference that cost a real
    send to find: git accepts any string, and the VAPID library does not. It
    requires something that could route, so a bare hostname is refused and the
    push fails with "Missing 'sub' from claims", which names neither the
    setting nor the reason.

    `.invalid` is reserved by RFC 2606 precisely for this: it has the shape of
...
def notifications_configured() -> bool:
    """Whether this machine can send a push at all.

    Both halves or neither: a private key with no public key cannot be
    subscribed to, and a public key with no private key cannot sign.
    """
