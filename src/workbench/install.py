...
#: Secrets and machine-wide settings, read by every unit that spends one.
...
#: library lives — this module imports nothing but the standard library, and
#: runs before a virtualenv exists at all.
_VAPID_SCRIPT = """
import base64
...
key = Vapid01()
key.generate_keys()
raw_private = key.private_key.private_numbers().private_value.to_bytes(32, "big")
raw_public = key.public_key.public_bytes(
    serialization.Encoding.X962, serialization.PublicFormat.UncompressedPoint
)
encode = lambda raw: base64.urlsafe_b64encode(raw).decode().rstrip("=")
print(encode(raw_private))
print(encode(raw_public))
"""


def ensure_notification_keys(account: pwd.struct_passwd) -> None:
    """Give this machine an identity to send push notifications with.

    Unlike the tailnet and the agent's login, nothing here needs a person: a
    VAPID keypair is generated locally and identifies the *sender* to push
    services. So it is automated, which is what the reproducibility rule asks
    of any step that can be.

    Written once and never rotated by a re-install. The public half travels
    into every subscription a browser makes, so regenerating would silently
    orphan every device that had subscribed — they would keep their old key,
    and every push would be rejected.
    """
    if vapid_private_key() is not None:
        info("push notification keys already configured")
        return

    existing = ""
    if ENV_FILE.is_file():
        existing = ENV_FILE.read_text()
        if "WORKBENCH_VAPID_PRIVATE_KEY" in existing:
            info(f"push notification keys already in {ENV_FILE}")
            return

    generated = service_run([str(_venv_bin("python")), "-c", _VAPID_SCRIPT])
    lines = [line.strip() for line in generated.stdout.splitlines() if line.strip()]
    if generated.returncode != 0 or len(lines) != 2:
        warn("could not generate push notification keys; notifications stay off.")
        info(f"    {(generated.stderr or '').strip()[:200]}")
        return

    private, public = lines
    ENV_FILE.parent.mkdir(parents=True, exist_ok=True)
    addition = (
        "\n# Web Push identity for this machine, generated by the installer.\n"
        "# The public half is handed to every browser that subscribes, so\n"
        "# replacing these silently orphans every device already subscribed.\n"
        f"WORKBENCH_VAPID_PRIVATE_KEY={private}\n"
        f"WORKBENCH_VAPID_PUBLIC_KEY={public}\n"
    )
    ENV_FILE.write_text(existing + addition)
    # Mode and owner match what the file is for: secrets the units read as the
    # service account, and nothing else on the machine needs to.
    ENV_FILE.chmod(0o600)
    os.chown(ENV_FILE, account.pw_uid, account.pw_gid)
    info(f"generated push notification keys into {ENV_FILE}")
