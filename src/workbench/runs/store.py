...
def create_task_conversation(
    ...
    `seed_message` is optional and unrelated to the two cases above: with
    nothing, the runner opens with `continuation_prompt`'s generic
    announcement, exactly as before this parameter existed. With something —
    free text from the Discuss dialog, or one of the Split/Check CI
    shortcuts — the runner opens with that instead. See
    `runner._prepare_conversation`.
    """
    run = Run(
        task_id=task.id,
