...
**Something tells you, on whatever device you said.** A run finishing, a plan waiting to
be read, and a run that stopped to ask are all things you learn about by having a page
open or by checking back — which is fine at a desk and useless on a phone. So a terminal
transition now sends a Web Push to the devices that asked for one.

- **Web Push rather than Tailscale device discovery**, because the browser already knows
  what a device is and hands over an endpoint that works from anywhere. It also does not
  reopen the no-ingress decision: a push is a POST *out* to the browser vendor's push
  service, and nothing has to reach in.
- **The trigger is `finish_run`**, which every terminal transition already passes through
  from both the runner and the web process. A hook per call site is a hook that gets
  missed — there are about twenty.
- **Two kinds, not one.** `run_finished` is news; `run_needs_you` — a question, or a plan
  awaiting review — is a thing only you can unblock. A phone worth waking for the second
  is not necessarily worth waking for the first, which is why a device subscribes to a
  list rather than a flag. Cancelling notifies nobody: you pressed the button.
- **One transport, deliberately not a seam.** The agent backend earned an adapter because
  a second vendor was a stated intention. No second transport has been asked for, and
  what the task actually wants — a new trigger being a new call site — is what a generic
  `notify()` already gives.
- **A push can never affect a run.** Every failure path ends in a log line and a note on
  the device row; a subscription the push service reports as gone is disabled rather than
  deleted, because silencing a device remotely is possible and re-subscribing it remotely
  is not.

Three things that were found rather than designed, all by sending a real push instead of a
stubbed one:

- **The web unit did not read `/etc/workbench/env`.** Only the deploy and run units did.
  The keys would have reached the runner and not the process that hands the browser a
  public key to subscribe with — so subscribing would have done nothing on a perfectly
  configured machine. That is the pull request token's failure exactly, from the other
  side, and the fix is the same one line.
- **A VAPID `sub` must look routable.** `mailto:workbench@<hostname>` is what
  `agent_git_identity` does for commits and git accepts anything; the push library does
  not, and refuses with "Missing 'sub' from claims", which names neither the setting nor
  the reason. `.invalid` is reserved by RFC 2606 for exactly this shape.
- **On iOS, push is delivered to an installed web app and never to a Safari tab.** The
  manifest already says `display: standalone`, so this works — from the home screen icon
  only, which is worth knowing before concluding it is broken.

The keypair is generated by the installer, because nothing about it needs a person — the
one automatable credential in a project whose other two need a browser. It is never
regenerated: the public half travels into every subscription a browser makes, so replacing
